...
//        return Cache::remember($cacheKey, now()->addSeconds(2), function () use ($param, $table, $queryDate, $dashboardType) {
//            return Laravel::table($table)
//                ->leftJoin('cancellations', function($join) use ($table, $dashboardType, $queryDate) {
//                    $join->on($table . '.number', '=', 'cancellations.number')
//                        ->on($table . '.user_id', '=', 'cancellations.user_id')
//                        ->where('cancellations.soft_delete', 'false')
//                        ->where('cancellations.booking', '=', $dashboardType)
//                        ->where('cancellations.start_date', '<=', $queryDate)
//                        ->where('cancellations.end_date', '>=', $queryDate);
//                })
//                ->whereNull('cancellations.number')
//                ->where($table . '.soft_delete', 'false')
//                ->where($table . '.state', 'active')
//                ->where($table . '.start_date', '<=', $queryDate)
//                ->where($table . '.end_date', '>=', $queryDate)
//                ->select($table . '.*')
//                ->get();
//        });
    }
    ...
    public static function showDailyReport($table, $day)
    {
        ...
        $query = $table::whereRaw('start_date < ? AND end_date > ?', [$day, $day])
            ->isNotCancelled()
            ->isNotDeactivated()
