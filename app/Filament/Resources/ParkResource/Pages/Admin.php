...
use App\Filament\Resources\DashboardResource\Model as ParkModel;
use App\Models\Spot;
use App\Models\User;
...
class Admin
{
    ...
    public static function getNumber(): Select
    {
        return Select::make('number')
            ->disabled(fn(Page $livewire) => $livewire instanceof EditPark)
            ->label('Spot number')
            ->options(Spot::all()->pluck('number', 'id'))
            ->required(fn(Page $livewire) => $livewire instanceof CreatePark)
     ...
     * @return TextColumn
     */
    public static function showSpot(): TextColumn
    {
    ...
    public static function showState(): BadgeColumn
    {
        return BadgeColumn::make('state')
            ...
            ->searchable()
            ->toggleable()
            ->colors(['danger' => 'inactive', 'success' => 'active', 'warning' => 'active + cancelled'])
