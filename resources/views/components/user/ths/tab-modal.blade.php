@if($selectedTicket)
    <div x-data="{ isClosing: false}" x-cloak>
        @teleport('#ths')
        <div
                class="fixed inset-0 flex items-center justify-center z-50 backdrop-blur-sm animate-[fade-in_0.3s_both]">
            <div
                    class="rounded-lg w-full max-w-4xl p-6 lg:p-8 shadow-full bg-weekend"
                    :class="{'animate-[fade-in_0.3s_both]': !isClosing}">
                <div class="flex justify-between items-center mb-4 ltr-direction">
                    ...
                    <h3 class="font-bold text-main-mode cursor-help text-center mx-auto" title="Ticket Number">
                        🎫
                ...
                <div>
                    @if($activeTab === 'request')
                        <!-- Request Tab -->
                        <div class="md:min-h-[300px] animate-[fade-in_0.3s_both]]">
                            <div class="space-y-12 mb-7 {{ isDarkMode() ? 'bg-gray-700' : 'bg-gray-100'}} p-4 rounded-lg">
                                <p>
                            ...
                            <div class="flex justify-content-evenly w-full p-4 pt-8 mt-8">
                                <div class="flex items-center cursor-help" title="وضعیت">
                                    ...
                                    @elseif($selectedTicket['status'] === 'in-progress')
                                        <i class="fas fa-spinner text-yellow-500 ml-2 animate-spin"
                                           title="در حال انجام"></i>
                                ...
                                @if(!empty($selectedTicket['assignee_files']))
                                    <div class="flex flex-wrap space-x-4 mt-4">
                                        ...
                                        @foreach($selectedTicket['assignee_files'] as $file)
                                            <div
                                                    class="w-16 h-16 bg-gray-400 border rounded-md overflow-hidden cursor-pointer">
                                                @if(Str::contains($file['file'], ['jpg', 'jpeg', 'png', 'gif']))
                                                    <img src="{{ asset($file['file']) }}"
                                                ...
                                                @else
                                                    <a href="{{ asset($file['file']) }}" target="_blank"
                                                       title="Download or view file">
                                                        <i class="fas fa-file text-gray-600 flex items-center justify-center w-full h-full"></i>
                                                    </a>
                                                @endif
                                            </div>
                                        @endforeach
