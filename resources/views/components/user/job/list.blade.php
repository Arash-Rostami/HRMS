    ...
    @foreach($jobs->chunk(4) as $chunks)
        <div class="flex flex-wrap md:flex-nowrap w-full farsi-module persol-farsi-font justify-between">
            @foreach($chunks as $job)
                <div class="w-1/2 md:w-1/4 text-center p-1 cursor-pointer"
                     style="min-width: 25% !important;"
                     title="برای جزییات بیشتر کلیک نمایید"
                >
                    <div class="flip-card card-job links-thumbnails h-auto w-full">
                        <div class="flip-card-inner h-auto">
                            <div class="flip-card-front">
                                @php
                                    [ $avatar, $genderTitle ] = match($job->gender) {
                                        'Male'   => ['/img/user/male_avatar.svg',    'جنسیت مرد'],
                                        'Female' => ['/img/user/female_avatar.svg',  'جنسیت زن'],
                                        'Any'    => ['/img/user/neutral_avatar.svg', 'جنسیت: هر دو'],
                                    };
                                @endphp
                        ...
                        <div class="flip-card-inner h-auto">
                            ...
                            <div class=" p-2 h-auto">
                                ...
                                <div class="share-post"
                                     title="این فرصت شغلی را با دیگران به اشتراک بگذارید">
                                </div>
                            </div>
                        </div>
                    </div>
