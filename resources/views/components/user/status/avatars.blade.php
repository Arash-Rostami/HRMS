...
@endphp
<div class="mb-4 md:ml-4 float-right text-sm md:text-base">
    برای ارسال پیامک روی نام‌ و برای مشاهده شماره داخلی تلفن روی تصاویر کلیک کنید.
</div>
{{-- Filter the result --}}
<div class="mb-4 float-left md:w-1/5 w-1/2" dir="ltr">
    <div class="relative" data-te-input-wrapper-init>
        ...
        <label for="exampleSearch2"
               class="pointer-events-none absolute left-3 top-0 mb-0 max-w-[90%] origin-[0_0] truncate pt-[0.37rem] leading-[1.6] text-gray-600 transition-all duration-200 ease-out peer-focus:-translate-y-[0.9rem] peer-focus:scale-[0.8] peer-focus:links-thumbnails peer-data-[te-input-state-active]:-translate-y-[0.9rem] peer-data-[te-input-state-active]:scale-[0.8] motion-reduce:transition-none {{ isDarkMode() ? 'text-gray-200' : '' }}">⌕ جستجو</label>
    </div>
</div>
            ...
            <div class="flex py-1 flex-wrap justify-content-evenly">
                @foreach($users as $user)
                    ...
                    @if($user->presence == $presenceType && !Str::startsWith($user->forename, 'Guest'))
                        <div
                            class="text-center items-center justify-center mx-1 avatar-container relative group {{ $details['fade'] }}">
                            <img src="{{ showUserProfile($user) }}"
                                 ...
                                 alt="Avatar"/>
                            <p class="text-neutral-500 dark:text-neutral-400 cursor-pointer" dir="ltr"
                               title="ارسال SMS 📲"
                               @click="
                                openSMSModal = true;
                                receptor= '{{ $user->profile->cellphone ?? '' }}';
                                message= 'ارسال پیامک به {{ $user->fullName }}'">
                                {{ $user->initials }}
    ...
    <div class="absolute bottom-0 left-0 right-0 top-0 bg-opacity-75 flex justify-center items-center"
        ...
        >
            ...
            <div class="mt-4 flex justify-center space-x-4">
                ...
                <button class="bg-red-500 text-white px-2 py-1 rounded-md" @click="openSMSModal = false">لغو
                </button>
                <button class="bg-green-500 text-white px-2 py-1 rounded-md" @click="sendSMS()">ارسال
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="absolute ignore-elements top-1 right-2 rounded" x-show="openSMSToast">
        <div class="bg-main-mode max-w-xs text-gray-800 rounded-lg px-4 py-2 slide-in-top">
            <div class="cursor-pointer z-50 float-right ">
                <i class="fas fa-window-close text-gray-900" title="لغو"></i>
            </div>
            <br>
